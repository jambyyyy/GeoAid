...
function PurokDashboard() {
  ...
  // Lazy-loaded the first time the Reports tab is opened, rather than on
  // every dashboard load — it's a separate, less-frequently-viewed query.
  useEffect(() => {
    if (activeItem !== "Reports" || reportsData || reportsLoading) return;
    ...
    const fetchReports = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/api/purok/reports/?username=${encodeURIComponent(username)}`
        );
        const data = await response.json();
        setReportsData(data);
      } catch (err) {
    ...
    fetchReports();
  }, [activeItem, username, reportsData, reportsLoading]);
  ...
  return (
    <div className="dashboard-page">
      ...
      <div className="dashboard-main">
        ...
        {activeItem === "Reports" && (
          <section className="panel">
            {reportsLoading && <p className="panel-note">Loading reports…</p>}
            {reportsError && <p className="panel-note">{reportsError}</p>}
            {reportsData && !reportsLoading && (
              <>
                <div className="stats-grid reports-stats-grid">
                  <div className="stat-card stat-success">
                    <span className="stat-value">{reportsData.totals.approved}</span>
                    <span className="stat-label">Approved (all time)</span>
                  </div>
                  <div className="stat-card stat-danger">
                    <span className="stat-value">{reportsData.totals.rejected}</span>
                    <span className="stat-label">Rejected (all time)</span>
                  </div>
                  <div className="stat-card stat-warning">
                    <span className="stat-value">{reportsData.totals.pending}</span>
                    <span className="stat-label">Pending review</span>
                  </div>
                  <div className="stat-card stat-info">
                    <span className="stat-value">{reportsData.totals.unregistered}</span>
                    <span className="stat-label">Unregistered households</span>
                  </div>
                </div>

                <h2>Weekly activity</h2>
                {reportsData.weeks.every((w) => !w.approved && !w.rejected && !w.registered && !w.unregistered) ? (
                  <p className="empty-state">No registration activity in this period yet.</p>
                ) : (
