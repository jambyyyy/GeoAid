...
function Dashboard() {
  ...
  const [claimForm, setClaimForm] = useState({ household_code: "", disaster_id: "", item_id: "", quantity: 1, notes: "" });
  ...
  useEffect(() => {
    const fetchRelief = async () => {
      try {
        const response = await fetch(`${API_URL}/api/barangay/relief/?username=${encodeURIComponent(username)}`);
        const data = await readApiResponse(response);
        if (!response.ok) throw new Error(data.message || "Could not load relief records.");
        setReliefData(data);
        setReliefError("");
      } catch (err) {
        console.error(err);
        setReliefError(err.message || "Unable to load relief distribution records.");
      }
    };
  ...
  useEffect(() => {
    const fetchEvacuation = async () => {
      try {
        ...
        console.error(err);
        setEvacuationError(err.message || "Unable to connect to the server.");
      }
    };
  ...
  const handleReliefClaim = async (event) => {
    event.preventDefault();
    if (!claimForm.household_code || !claimForm.item_id || Number(claimForm.quantity) < 1) return;
    setClaimingRelief(true);
    try {
      const response = await fetch(`${API_URL}/api/barangay/relief/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "claim_relief",
          username,
          household_code: claimForm.household_code,
          disaster_id: claimForm.disaster_id || null,
          items: [{ item_id: Number(claimForm.item_id), quantity: Number(claimForm.quantity) }],
          notes: claimForm.notes,
        }),
      });
      const data = await readApiResponse(response);
      if (!response.ok) throw new Error(data.message || "Unable to record relief claim.");
      setClaimForm({ household_code: "", disaster_id: "", item_id: "", quantity: 1, notes: "" });
      const refreshed = await fetch(`${API_URL}/api/barangay/relief/?username=${encodeURIComponent(username)}`);
      const refreshedData = await readApiResponse(refreshed);
      if (!refreshed.ok) throw new Error(refreshedData.message || "Claim recorded, but the list could not refresh.");
      setReliefData(refreshedData);
      setReliefError("");
      showToast(`Relief claim recorded — DIST-${data.distribution_id}`);
    } catch (err) {
    ...
    <div className="dashboard-page">
      ...
      <div className="dashboard-main">
        ...
        {activeItem === "Relief Distribution" && (
          <>
          <section className="panel relief-claim-panel">
            <div className="section-heading-row">
              <div><h2>Claim Relief Goods</h2><p className="panel-note">Claim against the central CSWD/LGU inventory for a confirmed household in {barangay || "your barangay"}.</p></div>
            </div>
            <form className="relief-claim-form" onSubmit={handleReliefClaim}>
              <label>Household<select value={claimForm.household_code} onChange={(e) => setClaimForm({ ...claimForm, household_code: e.target.value })} required><option value="">Select confirmed household</option>{(reliefData?.households || []).filter((h) => !h.claimed).map((h) => <option key={h.household_code} value={h.household_code}>{h.household_name} ({h.household_code})</option>)}</select></label>
              <label>Disaster event<select value={claimForm.disaster_id} onChange={(e) => setClaimForm({ ...claimForm, disaster_id: e.target.value })}><option value="">General relief / no event</option>{(reliefData?.disasters || []).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></label>
              <label>Relief item<select value={claimForm.item_id} onChange={(e) => setClaimForm({ ...claimForm, item_id: e.target.value })} required><option value="">Select available item</option>{(reliefData?.inventory || []).filter((i) => i.stock_on_hand > 0).map((i) => <option key={i.id} value={i.id}>{i.name} — {i.stock_on_hand} {i.unit} available</option>)}</select></label>
              <label>Quantity<input type="number" min="1" value={claimForm.quantity} onChange={(e) => setClaimForm({ ...claimForm, quantity: e.target.value })} required /></label>
              <label className="relief-notes">Notes<textarea value={claimForm.notes} onChange={(e) => setClaimForm({ ...claimForm, notes: e.target.value })} rows="2" placeholder="Optional distribution note" /></label>
              <button className="primary-btn" type="submit" disabled={claimingRelief || !(reliefData?.inventory || []).some((i) => i.stock_on_hand > 0)}>{claimingRelief ? "Recording…" : "Record claim"}</button>
            </form>
            <p className="panel-note">Only confirmed, unclaimed households are shown. Stock is deducted atomically when the claim is recorded.</p>
          </section>
          ...
          <section className="panel">
            <div className="table-scroll">
              <table className="data-table">
                ...
                <tbody>
                  {(reliefData?.distributions || [])
                    .filter((r) => !barangay || r.barangay === barangay)
                    .map((r) => (
                    <tr key={r.id}>
                      <td>{r.household_name} ({r.household_code})</td>
                      <td>{r.items.map((item) => `${item.name} × ${item.quantity} ${item.unit}`).join(", ")}</td>
                      <td>{new Date(r.distributed_at).toLocaleString()}</td>
                      <td>DIST-{r.id}</td>
            ...
            {reliefError && <p className="panel-note">{reliefError}</p>}
            {!reliefError && (reliefData?.distributions || []).filter((r) => !barangay || r.barangay === barangay).length === 0 && (
              <p className="panel-note">No relief distribution records have been recorded yet.</p>
            )}
          </section>
          </>
        )}
