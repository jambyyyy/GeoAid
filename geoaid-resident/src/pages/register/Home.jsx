...
function Home() {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      const storedMobileNumber = sessionStorage.getItem("geoaid_resident_mobile") || "";
      setMobileNumber(storedMobileNumber);

      try {
        const response = await fetch(
          `${API_URL}/api/resident/dashboard/?mobile_number=${encodeURIComponent(storedMobileNumber)}`
        );
  ...
  const handleBellClick = async () => {
    // Optimistically clear the badge so the click feels instant, then let
    // the backend actually mark the notifications read.
    setData((prev) => (prev ? { ...prev, unread_alerts: 0 } : prev));
    try {
      await fetch(`${API_URL}/api/resident/alerts/read/`, {
        method: "POST",
    ...
    <MobileShell>
      <div className="home-screen">
        <header className="home-header">
          ...
          <button type="button" className="bell-btn" aria-label="Notifications" onClick={handleBellClick}>
            <BellIcon />
        ...
        <div className="home-body">
          ...
          <section className="center-card">
            ...
            {nearest_center ? (
              ...
              <p className="center-meta">No evacuation center has been set up yet.</p>
            )}
          </section>
