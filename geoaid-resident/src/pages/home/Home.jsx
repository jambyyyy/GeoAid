...
// isn't reachable, so the screen still resembles the mockup.
const FALLBACK_DATA = {
  ...
  members: [
    { name: "Maria Santos", role: "Head of Household", flag: "4Ps" },
    { name: "Juan Santos", role: "Spouse · PWD", flag: "PWD" },
    { name: "Liza Santos", role: "Child · Age 16", flag: null },
  ],
};
            ...
            <div className="members-list">
              {members.map((m) => (
                <div key={m.name} className="member-row">
                  ...
                  {m.flag && <span className={`flag-badge ${FLAG_CLASS[m.flag] || ""}`}>{m.flag}</span>}
                </div>
              ))}
            </div>
          </section>
