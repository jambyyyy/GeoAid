...
class Household(models.Model):
    ...
    # Set by purok_review_household() when a Purok President approves/
    # rejects a registration. Lets purok_reports() bucket approvals and
    # rejections by the week they were actually reviewed, instead of the
    # week the household originally registered.
...
class EvacuationCenter(models.Model):
    ...
    # Added so resident_dashboard() can compute a real nearest-center
    # distance instead of a hardcoded value. Nullable/blank because
    # existing centers (and centers added via the admin without a map
    # picker) may not have coordinates set yet.
    gps_lat = models.FloatField(null=True, blank=True)
    gps_lng = models.FloatField(null=True, blank=True)
...
class ReliefItem(models.Model):
    """A relief good tracked by CSWD/LGU. Stock is maintained from
    recorded receipts and completed household distributions."""
    ...
    def __str__(self):
        return f"{self.name} ({self.stock_on_hand} {self.unit})"


class ReliefInventoryTransaction(models.Model):
    ...
    item = models.ForeignKey(
        ReliefItem, on_delete=models.PROTECT, related_name="transactions"
    )
...
class ReliefDistribution(models.Model):
    """One household relief-goods claim/checklist entry."""

    household = models.ForeignKey(
        Household, on_delete=models.PROTECT, related_name="relief_distributions"
    )
    disaster_type = models.ForeignKey(
        DisasterType,
    ...
    status = models.CharField(
        max_length=20,
        choices=[("claimed", "Claimed"), ("cancelled", "Cancelled")],
        default="claimed",
    )
    ...
    def __str__(self):
        return f"{self.household.household_code} - {self.status}"


class ReliefDistributionItem(models.Model):
    """Goods included in a household's relief distribution."""

    distribution = models.ForeignKey(
        ReliefDistribution, on_delete=models.CASCADE, related_name="items"
    )
    relief_item = models.ForeignKey(
        ReliefItem, on_delete=models.PROTECT, related_name="distribution_lines"
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["distribution", "relief_item"],
                name="unique_relief_item_per_distribution",
            )
        ]

    def __str__(self):
        return f"{self.relief_item.name} x {self.quantity}"


class Advisory(models.Model):
    """A DRRM/CSWD-issued advisory (e.g. a flood warning). Replaces the
    hardcoded 'Tibanga Gymnasium' / 'Flood Advisory — Tibanga' strings
    that used to be returned by purok_dashboard() and resident_dashboard().

    An empty `barangay` means the advisory is city-wide; otherwise it
    only applies to (and only generates notifications for) households
    in that one barangay. Managed via Django admin, same as DisasterType.
    """

    barangay = models.CharField(
        max_length=50, choices=Household.BARANGAY_CHOICES, blank=True,
        help_text="Leave blank for a city-wide advisory.",
    )
    disaster_type = models.ForeignKey(
        DisasterType, on_delete=models.SET_NULL, null=True, blank=True, related_name="advisories"
    )
    ...
    def __str__(self):
        return f"{self.title} ({self.barangay or 'City-wide'})"


class HouseholdNotification(models.Model):
    """One resident-facing alert, generated for a specific household when
    an Advisory is issued that covers their barangay (see the post_save
    signal in accounts/signals.py). Title/body are copied at creation time
    so editing or deactivating the source Advisory later doesn't rewrite
    a household's notification history.

    Backs resident_dashboard()'s `unread_alerts` count — previously a
    hardcoded `2` for every household regardless of whether anything had
    actually been issued.
    """

    household = models.ForeignKey(
        Household, on_delete=models.CASCADE, related_name="notifications"
    )
    advisory = models.ForeignKey(
        Advisory, on_delete=models.SET_NULL, null=True, blank=True, related_name="notifications"
    )
    title = models.CharField(max_length=150)
    body = models.TextField()
    ...
    def __str__(self):
        return f"{self.household.household_code} - {self.title} ({'read' if self.is_read else 'unread'})"
