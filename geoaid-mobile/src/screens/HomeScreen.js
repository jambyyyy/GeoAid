...
function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      const storedMobileNumber = (await AsyncStorage.getItem("geoaid_resident_mobile")) || "";
      setMobileNumber(storedMobileNumber);

      try {
        const response = await fetch(
          `${API_BASE}/api/resident/dashboard/?mobile_number=${encodeURIComponent(storedMobileNumber)}`
        );
  ...
  const handleBellPress = async () => {
    // Optimistically clear the badge so the tap feels instant, then let the
    // backend actually mark the notifications read. If the request fails,
    // the badge will just reappear next time the dashboard refetches.
    setData((prev) => (prev ? { ...prev, unread_alerts: 0 } : prev));
    try {
      await fetch(`${API_BASE}/api/resident/alerts/read/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mobile_number: mobileNumber }),
      });
    } catch (err) {
      console.warn("Couldn't mark alerts as read:", err.message);
    }
  };
  ...
  const occupancyPct = nearest_center && nearest_center.capacity
    ? Math.round((nearest_center.occupancy / nearest_center.capacity) * 100)
    : 0;
        ...
        <ScrollView contentContainerStyle={styles.body}>
          ...
          <View style={styles.section}>
            ...
            {nearest_center ? (
                ...
                <View style={styles.centerTop}>
                  <View>
                    ...
                    <Text style={styles.centerMeta}>
                      {nearest_center.distance_km != null
              ...
              <Text style={styles.centerMeta}>No evacuation center has been set up yet.</Text>
            )}
          </View>
